...
require(`../models/Dentist.js`);


//@desc Get all dentists 
//@routeGET /api/v1/dentists
//@access Public
    ...
    try{
        ...
        if (req.user.role=='admin'){
            query = Dentist.find(JSON.parse(queryStr)).populate({
                path:'bookings',
                options: {sort : {bookDate : 1}}
            });
        }
        else{
        query=Dentist.find(JSON.parse(queryStr));/*.populate({
            path : 'bookings',
            select: '-bookDate -_id -dentist',
            //options: {sort : {bookDate : 1}}
        });*/
        }
        //Select fields
...
//@routw POST /api/v1/dentists
//@access Private
exports.createDentist=async(req,res,next) => {
    const {name , year_of_experience} = req.body;

    let area_of_expertise;

     if (year_of_experience < 5) {
      area_of_expertise = "Newbie";
    } else if (year_of_experience <= 10) {
      area_of_expertise = "Intermediate";
    } else {
      area_of_expertise = "Expert";
    }

    const dentist = await Dentist.create({name , year_of_experience , area_of_expertise});
